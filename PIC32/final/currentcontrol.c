...
static volatile int Iactual;
static volatile int e_current = 0, e_INT = 0;

//extern volatile int Iref;

void PWM_init(void) {
  T2CONbits.TCKPS = 2;     // Timer2 prescaler N=1 (1:1)
...
void __ISR (_TIMER_2_VECTOR, IPL5SOFT) currentController (void){     // _TIMER_2_VECTOR = 8
  ...
  switch (MODE) {
    ...
    case ITEST:
    {
      Iactual = (1.841 * (ADC_avg()) - 1015.7);
      e_current = Waveform[counter] - Iactual;
      ...
      if (StoringData) {
        MAarray[counter] = Iactual;     // store data in global arrays
